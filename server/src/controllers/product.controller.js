...
export const addProduct = asyncHandler( async( req, res) => {
    const { name, description, category, offer } = req.body
    const user = req?.user
...
export const deleteProduct = asyncHandler( async( req, res) => {
    ...
    const imageDeletedFromCloudinary = await deleteFromCloudinary(product?.image)
...
export const getProductsByCategory = asyncHandler( async (req, res) => {
    ...
    res.status(200).json(
        new ApiResponse(200, products, "Products with category fetched successfully")
    )
